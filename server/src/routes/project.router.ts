...
apiProjectRouter.post('/projects/newproject', getProjectValidationRules(), validate, catchErrors(createProject)); //! A vérifier également
apiProjectRouter.get('/projects', catchErrors(findAllProjects));
apiProjectRouter.get('/projects/:id', catchErrors(findProjectById));
apiProjectRouter.patch('/projects/dashboard/:id', catchErrors(updateProjectById)); //! p-e ajouter les règles de validation
apiProjectRouter.delete('/projects', catchErrors(deleteProjectById)); //! Route à vérifier car générale
